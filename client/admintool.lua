...
RegisterNetEvent('rcore_cam:openAdminTool', function()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('admin') then
            ...
            local groupId, camId = nil, nil

            if selectedCamData then
                groupId, camId = FindExistingCamera(selectedCamData.pos)
            end

            local isExisting = groupId and camId

            if not tagInputOpen then
                ...
                elseif selectedCamData and isExisting and WarMenu.Button('Set Camera Look Here') then
                    local lookPos = GetEntityCoords(PlayerPedId())
                    SetCameraLookHere(selectedCamData, lookPos)

                    Citizen.CreateThread(function()
                        local scd = selectedCamData
                        local displayUntil = GetGameTimer() + 5000

                        while displayUntil > GetGameTimer() do
                            DrawLine(
                                scd.pos, 
                                lookPos, 
                                0, 255, 0, 200)
                ...
                elseif selectedCamData and isExisting and WarMenu.Button('Tag Camera') then
                    tagInputOpen = true
                    Citizen.CreateThread(function()
                        local selectedTags = HandleCamTagInput()

                        if selectedTags == -1 then
                            SetCameraTags(selectedCamData, nil)
                        elseif selectedTags then
                            SetCameraTags(selectedCamData, selectedTags)
                        end
...
function HandleCamTagInput()
    AddTextEntry('rc_cam_tag', "Enter camera tag")
    DisplayOnscreenKeyboard(false, "rc_cam_tag", "", "", "", "", "", 64)

    while true do
        onScreenKeyboardStatus = UpdateOnscreenKeyboard()
        if onScreenKeyboardStatus == 3 then
            return
        elseif onScreenKeyboardStatus == 1 then
        ...
        elseif onScreenKeyboardStatus == 2 then
            return nil
        elseif onScreenKeyboardStatus == -1 then
            return
        end
...
function SetCameraLookHere(selectedCamData, lookPos)
    local groupId, camId = FindExistingCamera(selectedCamData.pos)
    AllCameras[groupId][camId].lookAt = lookPos
end

function SetCameraTags(selectedCamData, selectedTags)
    local groupId, camId = FindExistingCamera(selectedCamData.pos)

    for _, c in pairs(AllCameras[groupId]) do
        c.tags = selectedTags
    end
end
